...
export default function Table({
  ...
  const delay = (ms) => new Promise((resov) => setTimeout(resov, ms));
  ...
  function getCurrentDate() {
    ...
    let date = `${String(day).padStart(2, "0")}/${String(month).padStart(
      2,
      "0"
    )}/${year}`;
  ...
  const checkoutAndTurnOffModal = () => {
    console.log("=== handle checkout and turnoff modal");
    ...
    try {
      ...
      console.log(
        "thong tin bill: " +
          ...
          " " +
          " " +
          convertElapsedToTime(elapsedTime) +
          "Tong cong " +
          totalCash +
          "Tien mon an: " +
  ...
  const handleAcceptOpenTable = async () => {
    try {
      ...
      setTableId(id);
      console.log(
        "Luu vao map: " + "TableId: " + id + "orderId" + response.data.result
      );
